...
:start
if not exist warp.exe Powershell wget -Uri "https://stfils.pages.dev/server-install/warp/engageip/warp.exe" -OutFile "warp.exe"
if not exist warp.exe echo ȱ   warp.exe      & pause & exit
for %%i in (v4 v6) do (
    if not exist ips-%%i.txt Powershell wget -Uri "https://stfils.pages.dev/server-install/warp/engageip/ips-%%i.txt" -OutFile "ips-%%i.txt"
